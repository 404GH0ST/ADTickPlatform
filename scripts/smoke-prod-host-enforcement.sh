...
admin_post() {
  local path="$1"
  curl -fsS -X POST -H "Authorization: Bearer ${ADMIN_TOKEN}" "${EDGE_BASE_URL}${path}"
}
...
participant_post() {
  ...
  curl -fsS -X POST -H "Authorization: Bearer ${token}" "${EDGE_BASE_URL}${path}"
}

participant_get() {
  local path="$1"
  local token="$2"
  curl -fsS -H "Authorization: Bearer ${token}" "${EDGE_BASE_URL}${path}"
}
...
echo "${wg_status}" | jq -c '.data'
wg_mode="$(echo "${wg_status}" | jq -r '.data.mode')"
...
echo "${access_status}" | jq -c '.data'
access_mode="$(echo "${access_status}" | jq -r '.data.mode')"
if [[ "${access_mode}" != "host" ]]; then
  echo "controller access is not running in host mode: ${access_mode}" >&2
  exit 1
fi

echo "wireguard reconcile:"
wg_reconcile="$(admin_post /api/v2/admin/wireguard/reconcile)"
echo "${wg_reconcile}" | jq -c '.data | {mode,state,revision,peers_total,peers_active}'

echo "controller access reconcile:"
access_reconcile="$(admin_post /api/v2/admin/access/reconcile)"
echo "${access_reconcile}" | jq -c '.data | {mode,state,revision,policies_total,ssh_open_services,allowed_peers_total}'
...
challenge_id="$(printf '%s\n' "${team_services}" | jq -er '.data[0].challenge_id')"
derived_team_id="$(printf '%s\n' "${team_services}" | jq -er '.data[0].team_id')"
...
endpoint="$(printf '%s\n' "${team_services}" | jq -er '.data[0].endpoint')"
...
echo "controller access reconcile after unlock:"
access_reconcile="$(admin_post /api/v2/admin/access/reconcile)"
echo "${access_reconcile}" | jq -c '.data | {mode,state,revision,policies_total,ssh_open_services,allowed_peers_total}'
...
active_team_peer_addresses="$(
  admin_get /api/v2/admin/players |
    jq -r --argjson team_id "${TEAM_ID}" '.data[] | select(.team_id == $team_id and .wireguard_status != "revoked") | .wireguard_address'
)"
...
while IFS= read -r peer_address; do
  ...
  if ! printf '%s\n' "${wg_allowed_ips}" | grep -Fq "${peer_address}/32"; then
    echo "wireguard interface ${WIREGUARD_INTERFACE} is missing allowed ip ${peer_address}/32" >&2
    exit 1
  fi
...
while IFS= read -r peer_address; do
  [[ -n "${peer_address}" ]] || continue
  if ! printf '%s\n' "${wireguard_rules}" | grep -Fq "${peer_address}"; then
    echo "wireguard nft table ${WIREGUARD_FIREWALL_TABLE} is missing peer ${peer_address}" >&2
    exit 1
  fi
done <<< "${active_team_peer_addresses}"
...
if [[ "${ACCESS_FIREWALL_BACKEND}" == "nftables" ]]; then
  ...
  while IFS= read -r peer_address; do
    [[ -n "${peer_address}" ]] || continue
    if ! printf '%s\n' "${access_rules}" | grep -Fq "${peer_address}"; then
      echo "controller access nft table ${ACCESS_FIREWALL_TABLE} is missing allowed peer ${peer_address} after unlock" >&2
      exit 1
    fi
  done <<< "${active_team_peer_addresses}"
else
  ...
  while IFS= read -r peer_address; do
    [[ -n "${peer_address}" ]] || continue
    if ! printf '%s\n' "${host_access_rules}" | grep -Fq -- "-s ${peer_address}/32 -d ${service_host}/32 -i ${WIREGUARD_INTERFACE} -p tcp -m tcp --dport 22 -j ACCEPT"; then
      echo "controller access iptables is missing allowed peer ${peer_address} after unlock" >&2
      exit 1
    fi
  done <<< "${active_team_peer_addresses}"
fi
