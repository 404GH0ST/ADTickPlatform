...
control_bind_address="${HOST_CONTROL_BIND_ADDRESS:-172.17.0.1}"
controller_metrics_port="${CONTROLLER_SERVICE_PORT:-18084}"
capture_host_metrics "controller-service" "http://${control_bind_address}:${controller_metrics_port}/metrics" "${output_dir}/controller-service-metrics.prom"
capture_service_metrics "realtime-gateway" "http://127.0.0.1:8086/metrics" "${output_dir}/realtime-gateway-metrics.prom"
wireguard_metrics_port="${WIREGUARD_GATEWAY_PORT:-18087}"
capture_host_metrics "wireguard-gateway" "http://${control_bind_address}:${wireguard_metrics_port}/metrics" "${output_dir}/wireguard-gateway-metrics.prom"
