...
export default defineConfig({
  ...
  webServer: [
    ...
    },
    {
      command: [
        "sh -lc '",
        `export AD_PLATFORM_API_URL=${mockApiUrl};`,
        `export AD_PLATFORM_GAME_CORE_URL=${mockApiUrl}/game-core;`,
        `export AD_PLATFORM_SUBMISSION_SERVICE_URL=${mockApiUrl}/submission-service;`,
        `export AD_PLATFORM_REALTIME_URL=${mockApiUrl};`,
        `export AD_PLATFORM_CONTROLLER_METRICS_URL=${mockApiUrl}/controller-service;`,
        `export AD_PLATFORM_WIREGUARD_GATEWAY_URL=${mockApiUrl}/wireguard-gateway;`,
        "export ADMIN_API_TOKEN=dev-admin-token;",
        `export TEAM_JWT_SECRET=${testTeamJWTSecret};`,
        "export NEXT_TELEMETRY_DISABLED=1;",
        "export NEXT_OUTPUT_MODE=default;",
        `bun run build && bun run start --hostname 127.0.0.1 --port ${appPort}`,
        "'",
      ].join(" "),
      url: appUrl,
      reuseExistingServer: !process.env.CI,
