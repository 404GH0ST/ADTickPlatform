...
It also captures `game-core-metrics.prom`, `submission-service-metrics.prom`, and `realtime-gateway-metrics.prom` from the live host stack so the validation artifacts include scheduler/checker, submission-path, and realtime health snapshots.
