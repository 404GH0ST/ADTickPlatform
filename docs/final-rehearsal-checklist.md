...
It also captures `game-core-metrics.prom` and `realtime-gateway-metrics.prom` from the live host stack so the validation artifacts include scheduler/checker and realtime health snapshots.
...
This uses the latest `.runtime/release-candidate-*` artifact tree by default, requires the nested go-live runtime alert snapshot to still be healthy, requires the nested attack-map load validation report to still show `passed`, requires the nested `game-core` and `realtime-gateway` metrics snapshots to still exist with the expected metric families, confirms the rendered event-ready note exists inside the artifact tree, and rejects the release if `git HEAD` does not match the validated commit.
