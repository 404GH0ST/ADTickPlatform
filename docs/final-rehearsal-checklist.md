...
It also captures `scoring-audit.json` and expects replayed scoring rows to match the stored scoreboard with zero mismatches.
It also writes `summary.json` so the final evidence set is machine-readable.
It also writes `operator-summary.json` so operators get a concise health view derived from the validated evidence set.
...
This uses the latest `.runtime/release-candidate-*` artifact tree by default, requires the nested go-live runtime alert snapshot to still be healthy, requires the nested scoring replay audit to report `ok` with zero mismatches, requires the nested attack-map load validation report to still show `passed`, requires the nested `game-core`, `submission-service`, `controller-service`, `realtime-gateway`, and `wireguard-gateway` metrics snapshots to still exist with the expected metric families, requires the generated `operator-report.html` to exist, confirms the rendered event-ready note exists inside the artifact tree, and rejects the release if `git HEAD` does not match the validated commit.
